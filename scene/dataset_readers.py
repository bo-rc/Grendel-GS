...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in tqdm(
        ...
        cam_name_split = extr.name.split("/")
        if len(cam_name_split) > 1:
            single_cam_folder = cam_name_split[0]
            img_name = cam_name_split[1]
            image_path = os.path.join(images_folder, single_cam_folder, img_name)
        else:
...
def readColmapSceneInfo(path, images, eval, llffhold=20, skip_cams=["cam5"]):
    ...
    cam_extrinsics = {image_id: image for image_id, image in cam_extrinsics.items() if not any(skip_cam in image.name for skip_cam in skip_cams)}
